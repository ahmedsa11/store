...
const AddCouponHook = () => {
    ...
    let PageCount=0
    try{
    if( allCoupon.paginationResult){
    PageCount=allCoupon.paginationResult.numberOfPages}
    else{
        PageCount =0}
}
catch(e) { }
    const getPage=(page)=>{
        dispatch(getAllCoupon(4,page))
    }
    return [coupnName, couponDate, couponValue, onChangeName, onChangeDate, onChangeValue, onSubmit, coupons,getPage,PageCount]
}
