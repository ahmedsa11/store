...
export const getOneOrders = (id) => async (dispatch) => {
  try {
    const response = await GetDataToken(`/api/v1/orders/${id}`);
...
export const changeOrderPay = (id) => async (dispatch) => {
  try {
    const response = await InsUpdateData(`/api/v1/orders/${id}/pay`);
...
export const changeOrderDeliver = (id) => async (dispatch) => {
  try {
    const response = await InsUpdateData(`/api/v1/orders/${id}/deliver`);
