...
export const addUserAddress = (body) => async (dispatch) => {
    try {
        const response = await InsertData("/api/v1/addresses", body);
        dispatch({
...
export const getAllUserAddress = () => async (dispatch) => {
    try {
        const response = await GetDataToken('/api/v1/addresses');
...
export const getOneUserAddress = (id) => async (dispatch) => {
    try {
        const response = await GetDataToken(`/api/v1/addresses/${id}`);
...
export const editUserAddress = (id, body) => async (dispatch) => {
    try {
        const response = await InsUpdateData(`/api/v1/addresses/${id}`, body);
