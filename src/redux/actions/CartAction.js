...
export const addProductToCart = (body) => async (dispatch) => {
    try {
        const response = await InsertData(`/api/v1/cart`, body);
        dispatch({
            type: ADD_TO_CART,
            payload: response,
        })
...
export const getAllUserCartItems = () => async (dispatch) => {
    try {
        const response = await GetDataToken(`/api/v1/cart`);
...
export const updateCartItem = (id, body) => async (dispatch) => {
    try {
        const response = await InsUpdateData(`/api/v1/cart/${id}`, body);
        //  console.log(response)
        dispatch({
...
export const applayCoupnCart = (body) => async (dispatch) => {
    try {
        const response = await InsUpdateData(`/api/v1/cart/applyCoupon`, body);
       // console.log(response)
        dispatch({
